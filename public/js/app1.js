...
var dhf = angular.module("dhf", ['autocomplete', 'ui.router', 'ngAnimate']);
...
dhf.controller("SearchCtrl", function($scope, $http, $stateParams, $location, $state) {
    ...
    $scope.aggregations = [];
    $scope.suggestion = [];
     alert(angular.toJson($stateParams, true));
    var current_path = '/';
    ...
    $scope.searchurl = Math.floor(Math.random() * (9999 - 100)) + 100;
            ...
            success(function(data, status) {
                ...
                if (!$scope.error) { // with results
                   ...
                   /* $scope.stNum = Math.ceil($scope.page * PAGE_SIZE - PAGE_SIZE) || 1;
                    $scope.endNum = Math.ceil($scope.page * PAGE_SIZE) || PAGE_SIZE;
                    if ($scope.endNum > $scope.total) {
                        $scope.endNum = $scope.total;
                    }
                    $scope.stPGNum = (parseInt($scope.page) - 5) < 0 ? 1 : parseInt($scope.page) - 5;
                    $scope.endPGNum = (parseInt($scopes.page) + 5) > Math.ceil($scope.total / PAGE_SIZE) ? ($scope.total / PAGE_SIZE) : parseInt($scope.page) + 5;*/
                    $scope.aggregations = [];
                    $scope.factreq = $scope.req;
                    $scope.factreq.fcfields = 'language,city,county,degree,group_name,network,specialties,acpt_new_pat';
                    ...
                    success(function(data1, status) {
                        ...
                        if (!$scope.error) { // with results
                            ...
                            $scope.aggregations['city'] = data1.response.city.terms || [];
                            $scope.aggregations['county'] = data1.response.county.terms || [];
                            $scope.aggregations['degree'] = data1.response.degree.terms || [];
                            ...
                            $scope.aggregations['acpt_new_pat'] = data1.response.acpt_new_pat.terms || [];
                        }
